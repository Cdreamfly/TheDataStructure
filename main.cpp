...
int main() {
    ...
    float time4 = testMap(hashTable, filename);
    std::cout << "HashTable time : " << time4 << " s " << std::endl;

    std::cout << "Test Dataes:" << std::endl;
    BSTMap<int, int> *bstMap1 = new BSTMap<int, int>();
    float time5 = testData(bstMap1, testDataes);
    std::cout << "BSTMap time : " << time5 << " s " << std::endl;
    RBTree<int, int> *rbTree1 = new RBTree<int, int>();
    float time6 = testData(rbTree1, testDataes);
    std::cout << "RBTree time : " << time6 << " s " << std::endl;
    AVLTree<int, int> *avlTree1 = new AVLTree<int, int>();
    float time7 = testData(avlTree1, testDataes);
    std::cout << "AVLTree time : " << time7 << " s " << std::endl;
    HashTable<int, int> *hashTable1 = new HashTable<int, int> ();
    float time8 = testData(hashTable1, testDataes);
    std::cout << "HashTable time : " << time8 << " s " << std::endl;

    std::cout << "Test Dataes1" << std::endl;
    RBTree<int, int> *rbTree2 = new RBTree<int, int>();
    float time9 = testData(rbTree2, testDataes1);
    std::cout << "RBTree time : " << time9 << " s " << std::endl;
    AVLTree<int, int> *avlTree2 = new AVLTree<int, int>();
    float time10 = testData(avlTree2, testDataes1);
    std::cout << "AVLTree time : " << time10 << " s " << std::endl;
    HashTable<int, int> *hashTable2 = new HashTable<int, int> ();
    float time11 = testData(hashTable2, testDataes1);
    std::cout << "HashTable time : " << time11 << " s " << std::endl;
